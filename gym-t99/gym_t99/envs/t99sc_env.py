...
class T99SC(gym.Env):
    ...
    def __init__(self, enemy, num_players=2):
        ...
        this environment uses action space simplified for agent. Each action is a dictionary
            action = {
                column: x,          # 0 <= x <= 9, where x is the target column for current piece
                rotation: r,        # 0 <= r <= 3, where r is the number of clockwise rotations a piece will make
                attack: a,          # 0 <= a <= 3 or a = -1, where a is the attack pattern agent currently uses
                                    # if a == -1, the attack pattern from the last action is perserved
                swap: s,            # s = True or s = False, where s sognifies whether the piece will be swapped
            }
